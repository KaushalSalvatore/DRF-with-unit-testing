...
from authAccount.serializers import UserSerializer
from django.views.decorators.csrf import ensure_csrf_cookie, csrf_protect
from django.utils.decorators import method_decorator
from django.conf import settings
from authAccount.utils import send_activation_email,send_reset_password_email
...
class LogoutView(APIView): # this logout view will delete our session id from database (from django_session table)
    def post(self, request):
        logout(request)
        return Response({'detail': 'Logged out successfully.'}, status=status.HTTP_200_OK)
# by default in logout csrf token required because it call after login 
...
class UserDetailView(APIView):
    ...
    def patch(self, request): # patch request for update user partial
...
class DeleteAccountView(APIView):
    def delete(self, request):
        user = request.user
        user.delete()
        logout(request) # this will also delete user session form django_session
